  ...
  return `
    <div class="border border-gray-100 rounded-2xl overflow-hidden shadow-sm bg-white flex flex-col justify-between">
      ...
      <div class="p-4 flex-grow flex flex-col justify-between">
        <div>
          <p class="text-[12px] text-willow-mid font-bold uppercase mb-1">${formatDate(excursion.date)} - ${excursion.location}</p>
          <h3 class="font-bold text-sm text-gray-800 mb-2">${excursion.title}</h3>
          <p class="text-sm text-gray-700 mb-4">${excursion.description}</p>
        </div>
        <a href="selected_activity.php?id=${excursion.id}"
          class="block text-center border border-willow-mid text-willow-dark py-2 rounded-xl text-sm font-semibold hover:bg-willow-cream transition">
          View Event
        </a>
      </div>
    </div>
...
async function load() {
  ...
  if (!excursions.length) {
    listEl.innerHTML = '<p class="text-sm text-gray-700 col-span-3">Sem excursões disponíveis.</p>'
    return
  }
